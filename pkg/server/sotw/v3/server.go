...
func (s *server) process(str stream.Stream, reqCh <-chan *discovery.DiscoveryRequest, defaultTypeURL string) error {
	...
	send := func(resp cache.Response) (string, error) {
		...
		if s.callbacks != nil {
			...
			s.callbacks.OnStreamResponseWithSnapshotVersion(resp.GetContext(), streamID, resp.GetRequest(), out, snapshotVersion)
		}
	...
	//		fmt.Printf("hahahaha %s",resp.(*cache.RawResponse).Version)
	//		if !more {
